...
def smiles_to_cas_api(smiles):
    ...
    try:
        ...
        cas_number = next((synonym.split(' ')[-1] for synonym in synonyms if re.match(r'^\d+-\d+-\d+$', synonym)), None)
        if cas_number:
            return cas_number
        else:
            return False
...
def structure_converter(structure, formate):
    url = f"https://cactus.nci.nih.gov/chemical/structure/{structure}/{formate}"
    try:
        response = requests.get(url)
...
# def smiles_to_cas_api(smiles):
#     return structure_converter(smiles,'cas')

def smiles_to_iupac_api(smiles):
    return structure_converter(smiles,'iupac_name')

def cas_to_iupac_api(cas):
    return structure_converter(cas,'iupac_name')

def iupac_to_smiles_api(iupac):
    return structure_converter(iupac,'smiles')

def cas_to_smiles_api(smiles):
    return structure_converter(smiles,'smiles')
